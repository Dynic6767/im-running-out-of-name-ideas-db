...
# Для хранения, кто уже получил приз (prize_id: set(user_id))
winners_cache = {}
...
def callback_query(call):
    try:
        ...
        if user_id in winners_cache.get(prize_id, set()):
        ...
        bot.answer_callback_query(call.id, "Произошла ошибка. Попробуй позже.")
...
def shedule_thread():
    schedule.every(5).seconds.do(send_message)  # Периодичность отправки
    while True:
...
if __name__ == '__main__':
    ...
    polling_shedule_obj = threading.Thread(target=shedule_thread, daemon=True)

    polling_thread_obj.start()
    polling_shedule_obj.start()

    # Чтобы главный поток не завершался
    polling_thread_obj.join()
    polling_shedule_obj.join()